...
const extractImages = (req, body) => {
  ...
  if (req.files.images?.length) {
    ...
    const newImgs = req.files.images.map(f => `/uploads/do-jang/${f.filename}`);
...
exports.getAll = async (req, res) => {
  try {
    const items = await DoJang.find().sort({ order: 1, createdAt: 1 });
...
exports.create = async (req, res) => {
  try {
    ...
    const item = await DoJang.create(body);
...
exports.update = async (req, res) => {
  try {
    const body = { ...req.body };
    extractImages(req, body);
    const item = await DoJang.findByIdAndUpdate(req.params.id, body, { new: true });
...
exports.remove = async (req, res) => {
  try {
    await DoJang.findByIdAndDelete(req.params.id);
